 ...
 * 类说明：应用于出行应用的小车平滑移动
 */
public class SmoothMoveActivity extends AppCompatActivity {
    ...
    /**
     * startMove 开始移动 startMove
     */
    public void startMove(View view){
        if(mPolyline==null){
            Toast.makeText(this,"请设置路线",Toast.LENGTH_SHORT).show();
            return;
        }
        //读取轨迹点
        ...
        //取轨迹的第一个点 作为 平移滑动的启动
        LatLng drivePoint=mPolyline.getPoints().get(0);
        //计算点到线的最短距离 返回的应该是 最近距离点的坐标和下标
...
//        textIndexList.add(2);
//
//        /*list 所有路线点的集合  colorlist 存储每个点的颜色*/
...
////                          .setCustomTextureIndex(textIndexList)
//                .addAll(list)
//                .useGradient(true)
//                .width(18));
//        LatLngBounds bounds=new LatLngBounds(list.get(0),list.get(list.size()-2));
//        /*把地图移到到以指定的经纬度范围的，并且在这个范围四周加上padding像素的填充  100 padding*/
...
//        LatLngBounds bounds = new LatLngBounds(list.get(0), list.get(list.size() - 2));
        ...
        mAMap.animateCamera(CameraUpdateFactory.newLatLngBounds( getLatLngBounds(list), 100));

    }
    protected LatLngBounds getLatLngBounds(List<LatLng> list) {
        LatLngBounds.Builder b = LatLngBounds.builder();
        b.include(list.get(0));
        b.include(list.get(list.size() - 2));
        return b.build();
    }
